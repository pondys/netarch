...
static uint32_t gLocalIp = MAKE_IPV4_ADDR(192,168,43,119);
...
static struct inout_ring *ringInstance(void)
{
	if (NULL == rInst)
	{
		...
		if (NULL == rInst)
		{
		    rte_exit(EXIT_FAILURE, "rte_malloc rTnst failed.\n");
		}
...
static struct rte_mbuf* ng_send_udp(struct rte_mempool *mbuf_pool, uint8_t *data, uint16_t length)
{
    ...
    if (!mbuf)
    {
        rte_exit(EXIT_FAILURE, "Could not rte_pktmbuf_alloc.\n");
    }
...
static int ng_encode_arp_pkt(uint8_t *msg, uint16_t opcode, uint8_t *dst_mac, uint32_t sip, uint32_t dip)
{
	...
	struct rte_ether_hdr *eth = (struct rte_ether_hdr *)msg;
	rte_memcpy(eth->s_addr.addr_bytes, gSrcMac, RTE_ETHER_ADDR_LEN);
	if (0 == strncmp((const char*)dst_mac, (const char*)gDefaultArpMac, RTE_ETHER_ADDR_LEN))
	{
...
static struct rte_mbuf* ng_send_arp(struct rte_mempool *mbuf_pool, uint16_t opcode, uint8_t *dst_mac, uint32_t sip, uint32_t dip)
{
    ...
    if (!mbuf)
    {
        rte_exit(EXIT_FAILURE, "Could not arp rte_pktmbuf_alloc.\n");
    }
								...
								uint32_t dip, uint16_t id, uint16_t seqnb)
{
	...
	//3 icmp
	struct rte_icmp_hdr *icmp = (struct rte_icmp_hdr *)(msg + sizeof(struct rte_ether_hdr) 
	   ...
	   uint32_t sip, uint32_t dip, uint16_t id, uint16_t seqnb)
{
    ...
    if (!mbuf)
    {
        rte_exit(EXIT_FAILURE, "Could not icmp rte_pktmbuf_alloc.\n");
    }
    mbuf->pkt_len = total_length;
    mbuf->data_len = total_length;
...
static void arp_request_timer_cb(__attribute__((unused)) struct rte_timer *tim, void *arg)
{
    ...
    for (i = 110; i < 120; i++)
    {
		...
		struct in_addr addr;
		addr.s_addr = dstip;
		printf("arp timer ---> src: %s \n", inet_ntoa(addr));

        struct rte_mbuf* arpbuf = NULL;
		uint8_t *dstmac = ng_get_dst_macaddr(dstip);
	    if (NULL == dstmac)
	    {
	        //arpaddr FF:FF:FF:FF:FF:FF
		    //etheraddr 00:00:00:00:00:00
...
static struct localhost * g_host = NULL;
...
static struct localhost* get_hostinfo_fromfd(int sockfd)
{
	...
	for (host = g_host; host != NULL; host = host->next)
	{
	    if (sockfd == host->fd)
	    {
	        return host;
										...
										uint8_t *data, uint16_t length)
{
    ...
    ng_encode_udp_apppkt(pkt_data, sip, dip, sport, dport, srcmac, dstmac, data, total_len);
...
static int udp_out(struct rte_mempool *mbuf_pool)
{
	...
	for (host = g_host; host != NULL; host = host->next)
	{
		...
		int nb_send = rte_ring_mc_dequeue(host->sndbuf, (void**)&ol);
		...
		{
			struct rte_mbuf *udpbuf = ng_udp_pkt(mbuf_pool, ol->sip, ol->dip, 
...
static int udp_server_entry(__attribute__((unused))  void *arg)
{
	...
	localaddr.sin_addr.s_addr = inet_addr("192.168.43.119"); // 0.0.0.0
...
int main(int argc, char *argv[])
{
    ...
    if (NULL == ring)
    {
        rte_exit(EXIT_FAILURE, "Error in ring buffer init.\n");
    }

	if (ring->in == NULL)
	{
	    ring->in = rte_ring_create("ng in ring", RING_SIZE, rte_socket_id(), RING_F_SP_ENQ | RING_F_SC_DEQ);
	}

	if (ring->out == NULL)
	{
	    ring->out = rte_ring_create("ng out ring", RING_SIZE, rte_socket_id(), RING_F_SP_ENQ | RING_F_SC_DEQ);
	}
#endif
